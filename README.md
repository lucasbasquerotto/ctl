...
The vault file used to decrypt the [project environment file](#) in the [project environment repository](#project-environment-repository). This file is optional, but recommended to be used if the environemnt file has secrets in it. It's also recommended to make the repository private. The encryption should be done with [ansible-vault](#encrypt-with-ansible).
...
For example, running `./run launch --dev demo` with the `vars.yml` the same as the [example](#) above, the generated file will be:
